...
/// <summary>Bounds a call to a local Ollama server.</summary>
/// <param name="Api">The Refit-generated Ollama surface.</param>
/// <param name="Options">The configured Ollama settings.</param>
[System.Diagnostics.DebuggerDisplay("OllamaClient: {Options.BaseUrl}")]
public sealed record OllamaClient(IOllamaApi Api, OllamaOptions Options) : IOpenAiCompatibleClient
{
    /// <inheritdoc/>
    /// <remarks>
    ...
    public async Task<HttpResponseMessage> SendChatAsync(NimChatRequest request, CancellationToken cancellationToken)
    {
        ...
        if (!Options.Enabled)
        {
            return OpenAiCompatibleClients.Disabled("Ollama");
        }
